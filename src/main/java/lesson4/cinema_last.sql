...
CREATE TABLE `film_tbl` (
  ...
  `duration_fld` tinyint(3) unsigned NOT NULL,
  PRIMARY KEY (`film_id`),
...
--
-- Temporary view structure for view `schedule_viev`
--

DROP TABLE IF EXISTS `schedule_viev`;
/*!50001 DROP VIEW IF EXISTS `schedule_viev`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `schedule_viev` AS SELECT 
 1 AS `num`,
 1 AS `name_fld`,
 1 AS `duration_fld`,
 1 AS `price_fld`,
...
INSERT INTO `seance_tbl` VALUES (1,'2020-10-10 07:00:00',1,1),(2,'2020-10-10 09:00:00',2,2),(3,'2020-10-10 11:00:00',4,3),(4,'2020-10-10 14:00:00',3,4);
...
--
-- Final view structure for view `schedule_viev`
--
...
/*!50001 VIEW `schedule_viev` AS select row_number() OVER (ORDER BY `seance_tbl`.`start_time_fld` )  AS `num`,`film_tbl`.`name_fld` AS `name_fld`,`film_tbl`.`duration_fld` AS `duration_fld`,`price_tbl`.`price_fld` AS `price_fld`,`seance_tbl`.`start_time_fld` AS `start_time_fld` from ((`film_tbl` join `seance_tbl` on((`film_tbl`.`film_id` = `seance_tbl`.`film_id`))) join `price_tbl` on((`seance_tbl`.`price_id` = `price_tbl`.`price_id`))) */;
...
-- Dump completed on 2021-03-19  0:57:29
